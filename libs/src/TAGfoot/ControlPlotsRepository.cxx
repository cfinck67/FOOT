...
void ControlPlotsRepository::PrintOutputNtuple(Bool_t isdata) {
  ...
  if(GlobalPar::GetPar()->IncludeBM()){
    ...
    Int_t evnum,hitplane,hitview,hitcell, time_acq, hitisselected;
    ...
    tree_out->Branch("BM_hitisselected",&hitisselected);
    ...
    if(!isdata){
      tree_out->Branch("BM_MC_hit_realrdrift", &realrdrift );
    ...
    for(Int_t k=0;k<ntuple_out.evnum.size();k++){
      for (Int_t i = 0; i< ntuple_out.BM_hit_rdrift.at(k).size(); i++){//loop on BM hits
        ...
        hitisselected=ntuple_out.BM_hit_isselected.at(k).at(i);
        ...
        if(!isdata){
          realrdrift=ntuple_out.BM_MC_hit_realrdrift.at(k).at(i);
