...
void TATWparGeo::InitGeo()  {
  ...
  m_nBar = SCN_STRIP_HEIGHT/SCN_STRIP_WIDTH;
  m_nLayer = SCN_NLAY;
  ...
  m_barThick_Lz =SCN_STRIP_THICK;        // ONLY plastic scint
  ...
  m_dimension = TVector3( SCN_STRIP_WIDTH*m_nBar, SCN_STRIP_WIDTH*m_nBar, length_Lz );
  ...
  // double barXdim;// = SCN_STRIP_WIDTH;	// 2 cm
  // double barYdim;// = SCN_STRIP_HEIGHT;	// 44 cm
  ...
  for (int k=0; k<m_nLayer; k++) {
    ...
    double barXdim = ( k%2 == 0 ? SCN_STRIP_WIDTH : SCN_STRIP_HEIGHT );
    double barYdim = ( k%2 == 0 ? SCN_STRIP_HEIGHT : SCN_STRIP_WIDTH );
...
void TATWparGeo::PrintRegions( string geoFileName ) {
  ...
  // geofile << "* ***Scintillator" << endl;
  cout << "* REGIONS ***Scintillator" << endl;
