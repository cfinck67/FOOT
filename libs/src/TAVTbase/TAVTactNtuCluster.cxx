/*!
 \file
 \version $Id: TAVTactNtuCluster.cxx,v 1.9 2003/06/22 10:35:48 mueller Exp $
...
Bool_t TAVTactNtuCluster::FindClusters(Int_t iSensor)
{
   ...
   for (Int_t tci = 0; tci < fListOfPixels->GetEntries(); tci++){ // loop over hit pixels
	  ...
	  fPSeed = (TAVTntuHit*)fListOfPixels->At(tci);
	  if (!fPSeed->Found()) {
	  ...
	  if ( ((TAVTntuHit*)fListOfPixels->At(tci))->GetPulseHeight() > 0) { //fCut->GetSeedPulseHeightToNoise() ) {
		 hitOK = AddPixels( tci);
		 
		 if ( hitOK == kTRUE ) {
			TAVTcluster* cluster = pNtuClus->NewCluster(iSensor);
			cluster->SetPlaneNumber(iSensor);
			for (Int_t i = 0; i < GetIndexArray().GetSize(); ++i) {
			   TAVTntuHit* pixel = (TAVTntuHit*)GetListOfPixels()->At(GetIndexArray()[i]);
...
//______________________________________________________________________________
//
void TAVTactNtuCluster::ComputePosition()
{
...
//______________________________________________________________________________
//  
Bool_t TAVTactNtuCluster::AddPixels(Int_t aPixelIndex)
{
   
   TAVTntuHit   *aNeighbour;
   Bool_t    valid = true;
   ...
   for (Int_t iPix = 0; iPix < fListOfPixels->GetEntries(); ++iPix) { // loop over hit pixels
	  aNeighbour = (TAVTntuHit*)fListOfPixels->At(iPix);
	  if ( iPix != aPixelIndex ) {
	  ...
	  for (Int_t idx = 0; idx < fIndexArray.GetSize(); ++idx) {
		 TAVTntuHit* pixelI = (TAVTntuHit*)fListOfPixels->At(fIndexArray[idx]);
		 for (Int_t j = 0; j < fListOfPixels->GetEntries(); ++j) {
			TAVTntuHit* pixelO = (TAVTntuHit*)fListOfPixels->At(j);
   ...
   if(fDebugLevel>2) 
	  printf("TAVTactNtuCluster:Analyse found %d valid neighbours\n", fIndexArray.GetSize());
...
//______________________________________________________________________________
//  
void TAVTactNtuCluster::ReorderPixels()
{
   ...
   TAVTntuHit *aPixel = 0x0;
   TAVTntuHit* bPixel = 0x0;
   for( Int_t iPix = 0; iPix < fIndexArray.GetSize(); iPix++) { // loop on pixels in candidate hit
	  ...
	  for( Int_t jPix = 0; jPix < fIndexArray.GetSize(); jPix++) {
		 ...
		 aPixel = (TAVTntuHit*)fListOfPixels->At( fIndexArray[ iPix] );
		 bPixel = (TAVTntuHit*)fListOfPixels->At( fIndexArray[ jPix] );
		 if(fDebugLevel>3) 
	  ...
	  if( nOfneighbours[ iPix] > nOfneighbours[ iSeed] || 
		 (nOfneighbours[ iPix] == nOfneighbours[ iSeed] && (aPixel->GetPixelColumn()<((TAVTntuHit*)fListOfPixels->At( fIndexArray[ iPix] ))->GetPixelColumn() || 
															aPixel->GetPixelLine()<((TAVTntuHit*)fListOfPixels->At( fIndexArray[ iSeed] ))->GetPixelLine()) ) ) { // force lower lefter seed
		 iSeed = iPix;
	  }
   ...
   fPSeed     = (TAVTntuHit*)fListOfPixels->At( fIndexArray[0] );
