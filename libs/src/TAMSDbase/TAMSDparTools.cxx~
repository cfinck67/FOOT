...
/*!
  \class TAVTparTools TAVTparTools.hxx "TAVTparTools.hxx"
  \brief tools parameters for vertex. **
*/

ClassImp(TAVTparTools);

//______________________________________________________________________________
TAVTparTools::TAVTparTools()
: TAGpara(),
  fFileStream(),
  fFileName(),
  fDebugLevel(0)
...
void TAVTparTools::ReadItem(TString& item)
{
   Int_t pos = -1;
   Char_t buf[255];
   TString key;
...
//_____________________________________________________________________________
void TAVTparTools::ReadItem(Int_t &arg)
{
   TString item;
   ReadItem(item);
   arg = item.Atoi();
...
//_____________________________________________________________________________
void TAVTparTools::ReadItem(Float_t &arg)
{
   TString item;
   ReadItem(item);
   arg = item.Atof();
...
//_____________________________________________________________________________
void TAVTparTools::ReadItem(Float_t* coeff, Int_t size,  const Char_t delimiter)
{
   TString key;
   ReadItem(key);
...
//_____________________________________________________________________________
void TAVTparTools::ReadVector3(TVector3 &arg)
{
...
//_____________________________________________________________________________
void TAVTparTools::ReadStrings(Char_t *aString, Int_t aLength)
{
   ...
   if (end == -1 || pos == -1 || end-pos-1 > aLength) {
	  cout << "TAVTparGeo: problem with reading configuration file, missing \" or line too long" << endl; 
	  return;
   }
...
//_____________________________________________________________________________
TString TAVTparTools::Normalize(const char* line)
{
...
//______________________________________________________________________________
Bool_t TAVTparTools::Open(const TString& name) 
{
   // openning file
