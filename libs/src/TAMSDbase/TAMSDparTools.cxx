...
/*!
  \class TAMSDparTools TAMSDparTools.hxx "TAMSDparTools.hxx"
  \brief tools parameters for vertex. **
*/

ClassImp(TAMSDparTools);

//______________________________________________________________________________
TAMSDparTools::TAMSDparTools()
: TAGpara(),
  fFileStream(),
  fFileName(),
  fDebugLevel(0)
...
void TAMSDparTools::ReadItem(TString& item)
{
   Int_t pos = -1;
   Char_t buf[255];
   TString key;
...
//_____________________________________________________________________________
void TAMSDparTools::ReadItem(Int_t &arg)
{
   TString item;
   ReadItem(item);
   arg = item.Atoi();
...
//_____________________________________________________________________________
void TAMSDparTools::ReadItem(Float_t &arg)
{
   TString item;
   ReadItem(item);
   arg = item.Atof();
...
//_____________________________________________________________________________
void TAMSDparTools::ReadItem(Float_t* coeff, Int_t size,  const Char_t delimiter)
{
   TString key;
   ReadItem(key);
...
//_____________________________________________________________________________
void TAMSDparTools::ReadVector3(TVector3 &arg)
{
...
//_____________________________________________________________________________
void TAMSDparTools::ReadStrings(Char_t *aString, Int_t aLength)
{
   ...
   if (end == -1 || pos == -1 || end-pos-1 > aLength) {
	  cout << "TAMSDparGeo: problem with reading configuration file, missing \" or line too long" << endl; 
	  return;
   }
...
//_____________________________________________________________________________
TString TAMSDparTools::Normalize(const char* line)
{
...
//______________________________________________________________________________
Bool_t TAMSDparTools::Open(const TString& name) 
{
   // openning file
