...
/*!
  \class TAITparTools TAITparTools.hxx "TAITparTools.hxx"
  \brief tools parameters for vertex. **
*/

ClassImp(TAITparTools);

//______________________________________________________________________________
TAITparTools::TAITparTools()
: TAGpara(),
  fFileStream(),
  fFileName(),
  fDebugLevel(0)
...
void TAITparTools::ReadItem(TString& item)
{
   Int_t pos = -1;
   Char_t buf[255];
   TString key;
...
//_____________________________________________________________________________
void TAITparTools::ReadItem(Int_t &arg)
{
   TString item;
   ReadItem(item);
   arg = item.Atoi();
...
//_____________________________________________________________________________
void TAITparTools::ReadItem(Float_t &arg)
{
   TString item;
   ReadItem(item);
   arg = item.Atof();
...
//_____________________________________________________________________________
void TAITparTools::ReadItem(Float_t* coeff, Int_t size,  const Char_t delimiter)
{
   TString key;
   ReadItem(key);
...
//_____________________________________________________________________________
void TAITparTools::ReadVector3(TVector3 &arg)
{
...
//_____________________________________________________________________________
void TAITparTools::ReadStrings(Char_t *aString, Int_t aLength)
{
   ...
   if (end == -1 || pos == -1 || end-pos-1 > aLength) {
	  cout << "TAITparGeo: problem with reading configuration file, missing \" or line too long" << endl; 
	  return;
   }
...
//_____________________________________________________________________________
TString TAITparTools::Normalize(const char* line)
{
...
//______________________________________________________________________________
Bool_t TAITparTools::Open(const TString& name) 
{
   // openning file
