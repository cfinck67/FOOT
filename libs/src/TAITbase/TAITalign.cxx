/*!
 \file
 \version $Id: TAITalign.cxx,v 1.9 2003/06/22 10:35:48 mueller Exp $
 \brief   Implementation of TAITalign.
 */
#include "TMath.h"
...
TAITalign* TAITalign::fgInstance = 0x0;

//__________________________________________________________
TAITalign* TAITalign::Instance(const TString name, Int_t type)
{
   if (fgInstance == 0x0) 
	  fgInstance = new TAITalign(name, type);
...
//------------------------------------------+-----------------------------------
//! Default constructor.
TAITalign::TAITalign(const TString name, Int_t type)
: TObject(),
  fAlignMonitor(0x0),
  fAlignTracks(100),
  fTinyBound(150),
  ...
  fDebugLevel(0)
{
   ...
   if (type == 0)
	  fpEvent = new TAGdataDsc("vtEvent", new TAGmbsEvent());
   	  ...
   	  fpEvent = new TAGdataDsc("vtEvent", new TAITrawEvent());
	  confFile = "TAITdetector_Cat.cfg";
	  geoFile  = "TAITdetector_Cat0.map";
   } else {
	  printf("Undefined type for event\n");
	  return;
   }
   
   fpParMap    = new TAGparaDsc("vtMap", new TAITparMap());
   
   fpGeoMap    = new TAGparaDsc("vtGeo", new TAITparGeo());
   TAITparGeo* geoMap   = (TAITparGeo*) fpGeoMap->Object();
   geoMap->FromFile(geoFile.Data());
   
   fpConfig  = new TAGparaDsc("vtConf", new TAITparConf());
   TAITparConf* parconf = (TAITparConf*) fpConfig->Object();
   parconf->FromFile(confFile.Data());
   
   fpDatRaw    = new TAGdataDsc("vtDat", new TAITdatRaw());
   fpNtuRaw    = new TAGdataDsc("vtRaw", new TAITntuRaw());
   fpNtuClus   = new TAGdataDsc("vtClus", new TAITntuCluster());
   fpNtuTrack  = new TAGdataDsc("vtTrack", new TAITntuTrack());
   
   if (type == 0) {
	  fActEvtReader = new TAGactMbsReader("vtActEvtReader");
	  fActEvtReader->SetupChannel(fpEvent,  new TAGmbsEventFilterType(42));
	  fActDatRaw  = new TAITactDatRaw("vtActRaw", fpDatRaw, fpEvent, fpGeoMap, fpConfig);   
   } else if (type == 1) {
	  fActEvtReader = new TAITactAscReader("vtActEvtReader", fpEvent, fpConfig);
	  fActDatRaw  = new TAITactAscRaw("vtActRaw", fpDatRaw, fpEvent, fpGeoMap);   
   }
   fActEvtReader->Open(name.Data());
   
   fActNtuRaw  = new TAITactNtuRaw("vtActNtu", fpNtuRaw, fpDatRaw, fpParMap, fpGeoMap);
   
   fActClus    = new TAITactNtuClusterF("vtActClus", fpNtuRaw, fpNtuClus, fpConfig, fpGeoMap);
   fActTrack   = new TAITactNtuTrack("vtActTrack", fpNtuClus, fpNtuTrack, fpConfig, fpGeoMap);
   ...
   for (Int_t i = 0; i < fSecArray.GetSize(); ++i) {
	  fAlign[i] = new TAITntuAlign(fSecArray[i]);
...
// 
void TAITalign::SetBounding(Float_t bound)
{
   fTinyBound = bound;
   for (Int_t i = 0; i < fSecArray.GetSize(); ++i) {
	  fAlign[i]->SetBounding(fTinyBound);
...
//_____________________________________________________________________________
// 
void TAITalign::CreateHistograms()
{
...
//_____________________________________________________________________________
// 
void TAITalign::LoopEvent(Int_t nEvts)
{
   for( Int_t iPl = 0; iPl < fSecArray.GetSize(); iPl++) {
...
//______________________________________________________________________________
//
void TAITalign::Align(Int_t idx, TAITtrack* aTrack)
{
   ...
   if (nclus > 0) {
	  ...
	  for (Int_t k = 0; k < nclus; ++k) { // loop on hits
		 TAITcluster* cluster = pNtuClus->GetCluster(iPlane, k);
		 ...
		 if( fDebugLevel >= 2 ) 
			printf("   TAITalign::Align plane %d Trying hit %d and distance (%f,%f)=%f\n", iPlane, k, tDistanceU, tDistanceV, tDist2D);
		 
	  } // end loop on hits
	  
	  if( fDebugLevel >= 1 )
		 printf("   TAITalign::Align plane %d Accumulating with distance (%f,%f)=%f\n",iPlane, tMinDistanceU, tMinDistanceV, tMinDistance2D);
...
//______________________________________________________________________________
//
void TAITalign::UpdateTransfo(Int_t idx)
{
   TAITparGeo* pGeoMap  = (TAITparGeo*)fpGeoMap->Object();
   Int_t       iPlane   = fSecArray[idx];
...
//______________________________________________________________________________  
//
void TAITalign::UpdateGeoMapsUVW(fstream &fileIn, fstream &fileOut, Int_t idx)
{
   Char_t tmp[255];
   TString key;
   TAITparGeo* pGeoMap  = (TAITparGeo*)fpGeoMap->Object();
...
//______________________________________________________________________________  
//
Bool_t TAITalign::IsForAlignment(TAITtrack* track)
{
   Bool_t isOk = false;
   if (fIsFullAlignment) {
