/*!
 \file
 \version $Id: TAITactNtuPostTrack.cxx,v 1.9 2003/06/22 10:35:48 mueller Exp $
 \brief   Implementation of TAITactNtuPostTrack.
 */
...
#include "TAITntuVertex.hxx"

#include "TAITactNtuPostTrack.hxx"

/*!
 \class TAITactNtuPostTrack 
 \brief NTuplizer for vertex tracks. **
 */

ClassImp(TAITactNtuPostTrack);

//------------------------------------------+-----------------------------------
//! Default constructor.
TAITactNtuPostTrack::TAITactNtuPostTrack(const char* name, 
								 TAGdataDsc* pNtuClus, TAGdataDsc* pNtuTrack, 
								 TAGparaDsc* pConfig, TAGparaDsc* pGeoMap, TAGdataDsc* pNtuVertex)
: TAITactBaseNtuTrack(name, pNtuClus, pNtuTrack, pConfig, pGeoMap),
  fpNtuVertex(pNtuVertex)
{
  // AddDataIn(pNtuVertex, "TAITntuVertex");
}

//------------------------------------------+-----------------------------------
//! Destructor.
TAITactNtuPostTrack::~TAITactNtuPostTrack()
{
...
//  
Bool_t TAITactNtuPostTrack::Action()
{
   // VTX
...
//_____________________________________________________________________________
//  
Bool_t TAITactNtuPostTrack::FindTiltedTracks()
{
   ...
   TAITntuTrack*   pNtuTrack  = (TAITntuTrack*)   fpNtuTrack->Object();
   TAITntuVertex*  pNtuVertex = (TAITntuVertex*)  fpNtuVertex->Object();
   ...
   array.Clear();
   TVector3 posG;
   TAITcluster* vtxClus = new TAITcluster();
   ...
   while (++curPlane < nPlane) {
	  ...
	  for( Int_t iFirstClus = 0; iFirstClus < nClusters; ++iFirstClus) { // loop on cluster of last plane, 
		 ...
		 TAITcluster* cluster = (TAITcluster*)list->At( iFirstClus );
...
//_____________________________________________________________________________
//  
Bool_t TAITactNtuPostTrack::AppyCuts(TAITtrack* track)
{
   if (!TAITactBaseNtuTrack::AppyCuts(track))
	  return false;

   Float_t minDistance = fSearchClusDistance*(1 + 2.*TMath::Tan(track->GetTrackLine().GetTheta()*TMath::DegToRad()));
   
   TAITcluster* aCluster = track->GetCluster(0);
