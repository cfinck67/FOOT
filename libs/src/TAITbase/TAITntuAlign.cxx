...
ClassImp(TAITntuAlign) // Description of TAITntuAlign 

//______________________________________________________________________________
//  
TAITntuAlign::TAITntuAlign()
: TAGdata()
{
  SetDebug( 0 );
  fPlaneNumber = 0;
...
//______________________________________________________________________________
//  
TAITntuAlign::TAITntuAlign( Int_t aPlaneNumber)
: TAGdata()
{
  fDebugAlign       = 0;
  ...
  fBound            = 5000.; // preliminary max distance track-hit
   ...
   if(fDebugAlign) cout << endl << " -*-*- TAITntuAlign User Constructor -*-*- " << endl;
  ...
  if(fDebugAlign) cout << endl << " -*-*- TAITntuAlign User Constructor DONE -*-*- " << endl;
...
TAITntuAlign::~TAITntuAlign()
{ 
  // Default TAITntuAlign destructor.
}

//______________________________________________________________________________
//  
void TAITntuAlign::SetBounding(Double_t aBound)  
{ 
...
//______________________________________________________________________________
//  
void TAITntuAlign::Accumulate2D(TVector3& aTrackPos, Double_t aDistanceU, Double_t aDistanceV)
{
  ...
  if( fDebugAlign>1) 
	 printf("   TAITntuAlign: plane %d Accumulating in U and V with distance (%f, %f) bound %.1f\n", fPlaneNumber, aDistanceU, aDistanceV, fBound);
  ...
  // Accumaltion of clusters
   if ( TMath::Abs(aDistanceU) <= fBound && TMath::Abs(aDistanceV) <= fBound ) {
    ...
    fAlignCount2D++;
	 ...
	 if (fDebugAlign)
		 printf("   TAITntuAlign::Accumulate2D plane %d, dist=(%.1f, %.1f) / %.1f, t=(%.1f, %.1f), count2D=%d/%d\n", 
  ...
  if (fAlignCount2D > fNumberOfTracks) {
    ...
    fEnough2D = kTRUE;
    if (fDebugAlign) 
	   printf("   TAITntuAlign::Accumulate2D plane %d uoffset=%.1f +/- %.1f Voffset=%.1f +/- %.1f, Uresidu=%.1f +/- %.1f, Vresidu=%.1f +/- %.1f\n", 
...
//______________________________________________________________________________
//  
void TAITntuAlign::ShowCorrection()
{
