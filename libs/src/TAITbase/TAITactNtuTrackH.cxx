...
ClassImp(TAITactNtuTrackH);

//------------------------------------------+-----------------------------------
//! Default constructor.
TAITactNtuTrackH::TAITactNtuTrackH(const char* name, 
...
//------------------------------------------+-----------------------------------
//! Destructor.
TAITactNtuTrackH::~TAITactNtuTrackH()
{
...
//  
Bool_t TAITactNtuTrackH::FindTiltedTracks()
{
	...
	for (Int_t iPl = nPlane; iPl >=0; --iPl) { // loop on all planes	
		...
		for( Int_t iClus = 0; iClus < nClusters; ++iClus) { // loop on clusters of each plane
			...
			TAITcluster* Cluster = (TAITcluster*)ListClus->At(iClus);
			...
			for (UInt_t ibin = 0; ibin < vectMapListOfBinsXZ.size(); ++ibin) {	// Make separate Hough Transform for each identified bin in XZ Histo
				...
				for (unsigned int iVecMap = 0; iVecMap < vectMapXZ.size() ; ++iVecMap) {
					if (int(vectMapXZ[iVecMap]/1000) == 9) { // Case of vertices
						...
						TAITcluster* Cluster = (TAITcluster*)ListClus->At(vectMapXZ[iVecMap]%1000);
							...
							for (UInt_t ibinYZ = 0; ibinYZ < vectMapListOfBinsYZ.size(); ++ibinYZ) {
								...
								for (unsigned int iVecMap = 0; iVecMap < vectMapYZ.size() ; ++iVecMap) {
									if (int(vectMapYZ[iVecMap]/1000) == 9) { // Case of vertices
										...
										TAITcluster* Cluster = (TAITcluster*)ListClus->At(vectMapYZ[iVecMap]%1000);
								...
								if (MapPlane.count(9) && nHitPlane >= fRequiredClusters){ 
									...
									for (UInt_t iclusTr = 0; iclusTr < vecListClusTrack.size(); ++iclusTr) {
										...
										TAITcluster* iClusterTr = (TAITcluster*)iListTr->At(vecListClusTrack[iclusTr]%1000);
										iClusterTr->SetFound();
									}
								}
									...
									for (Int_t nclus = 0; nclus < track->GetClustersN(); ++nclus) {
										...
										TAITcluster* iCluster = (TAITcluster*)ListClus->At(track->GetCluster(nclus)->GetNumber());
				...
				for (UInt_t iclus = 0; iclus < vecListClus.size(); ++iclus) {
					...
					TAITcluster* iCluster = (TAITcluster*)iList->At(vecListClus[iclus]%1000);
...
//_____________________________________________________________________________
//  
Bool_t TAITactNtuTrackH::IsGoodCandidate(TAITtrack* track)
{
...
//_____________________________________________________________________________
//  
Bool_t TAITactNtuTrackH::FindVertices()
{
   TAITntuTrack* pNtuTrack = (TAITntuTrack*) fpNtuTrack->Object();
   ...
   if (nTracks > 1) {
	  for (Int_t i = 0; i < nTracks; ++i) {
		 for (Int_t j = i+1; j < nTracks; ++j) {
			TAITtrack* track0 = pNtuTrack->GetTrack(i);
			TAITtrack* track1 = pNtuTrack->GetTrack(j);
			TVector2 pos = track0->DistanceMin(track1);
			if (TMath::Abs(pos.X()) < TAITparGeo::GetTargetWidth()/2. && pos.Y() < fSearchClusDistance) {
			   TVector3* vec = new TVector3(0.,0.,0.);
			   TAITline line  = track0->GetTrackLine();
