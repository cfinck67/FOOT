/*!
  \file
  \version $Id: adapted from TABMactNtuRaw.cxx,v 1.9 2003/06/22 10:35:48 mueller Exp $
  \brief   Implementation of TADCactNtuRaw.
*/
...
#include "TADCactNtuRaw.hxx"

/*!
  \class TADCactNtuRaw TADCactNtuRaw.hxx "TADCactNtuRaw.hxx"
  \brief NTuplizer for DC raw hits. **
*/

ClassImp(TADCactNtuRaw);

//------------------------------------------+-----------------------------------
//! Default constructor.

TADCactNtuRaw::TADCactNtuRaw(const char* name,
			     TAGdataDsc* p_nturaw, 
			     TAGdataDsc* p_datraw, 
			     TAGdataDsc* p_timraw, 
			     TAGdataDsc* p_triraw, 
			     TAGparaDsc* p_geomap, 
			     TAGparaDsc* p_parcon)
  : TAGaction(name, "TADCactNtuRaw - NTuplize DC raw data"),
    fpNtuRaw(p_nturaw),
    fpDatRaw(p_datraw),
    fpTimRaw(p_timraw),
    fpTriRaw(p_triraw),
...
TADCactNtuRaw::~TADCactNtuRaw()
{}

void TADCactNtuRaw::cleanup() {
...
vector<double> TADCactNtuRaw::retrieve_V() {
...
vector<double> TADCactNtuRaw::retrieve_U() {
...
Bool_t TADCactNtuRaw::Action()
{
  TADCdatRaw* p_datraw = (TADCdatRaw*) fpDatRaw->Object();
  TAIRdatRaw* p_timraw = (TAIRdatRaw*) fpTimRaw->Object();
  TATRdatRaw* p_triraw = (TATRdatRaw*) fpTriRaw->Object();
  TADCntuRaw* p_nturaw = (TADCntuRaw*) fpNtuRaw->Object();
  TADCparGeo* p_geomap = (TADCparGeo*) fpGeoMap->Object();
  ...
  for (Int_t i = 0; i < i_nhit; i++) {
    const TADCrawHit& hit = p_datraw->Hit(i);
    ...
    Double_t i_timdc = hit.Time()-(irtime);
    if(i_timdc<0) 
    ...
    if(exi[i_cell][i_plane][tmp_view]==-1) {
      ...
      TADCntuHit *tm = p_nturaw->Hit(exi[i_cell][i_plane][tmp_view]);
      if(tm->Timdc()>i_timdc) {
        doReplace = kTRUE;
      }
    }
    ...
    if(i_drift<0) {
      Info("Action()","Negative Distances!!! %lf %lf %lf %lf",i_timdc,hit.Time()-(irtime)-t0_corr,(hit.Time()-(irtime)-t0_corr)*vdrift,stcorr);    
      //      i_drift = 0;
    }
    //V view, (X,Z)
	...
	u_cellPla[i_plane] = i_cell;
      ...
      TADCntuHit *mytmp = new((*(p_nturaw->h))[tmp_hi]) 
	TADCntuHit(i_iddc,   i_view,
		   i_plane,   i_cell,  
		   h_x, h_y, h_z,
		   h_cx, h_cy, h_cz,
		   i_drift,  i_time,
...
//=============================================================================
void TADCactNtuRaw::CalcEffi(double nume, double deno, double &eff, double &erro) {
