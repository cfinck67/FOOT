...
Bool_t TADCactNtuTrack::Action()
{
  ...
  if(trackHitsTop>2 && trackHitsSide>2) {
    //  if(trackHits>6) {
    stat = aTr.EstimateTrackPar(p_ntuhit,p_dcgeo);
    stat = aTr.ComputeDataAll(p_ntuhit);    
    
    //Chi2 fit
    p_ntutrk->Chi2Fit(p_ntuhit,&aTr,3);
    //Sort double hits
    sortDoubleHits(p_ntuhit,p_ntutrk,p_dcgeo,aSrTr,aTr,DoubleHit,trackHits2);
	...
	TADCntuHit* p_hitT = p_ntuhit->Hit(j);
	plane = p_hitT->Plane();
	view = p_hitT->View();
	view > 0 ? my_view = 0 : my_view = 1; 
	...
	if(!my_view && trackHitsTop>3 && p_hitT->TrkAss()) {
	  ...
	  refitImprovedTrackTop (p_ntuhit,p_ntutrk,p_dcgeo,trackHitsTop,trackHitsSide,aSrTr.GetChi2()*nass,aTr2,j);
      ...
      if(k>-1)
	refitImprovedTrackTop (p_ntuhit,p_ntutrk,p_dcgeo,trackHitsTop,trackHitsSide,aSrTr.GetChi2()*nass,aTr2,k);
	...
	for(int j = 0; j<i_nhit; j++) {
	  ...
	  if(my_view && trackHitsSide>3 && p_hitT->TrkAss()) {
	    ...
	    refitImprovedTrackSide(p_ntuhit,p_ntutrk,p_dcgeo,trackHitsTop,trackHitsSide,aSrTr.GetChi2()*nass,aTr2,j);
	...
	if(k>-1)
	  refitImprovedTrackSide (p_ntuhit,p_ntutrk,p_dcgeo,trackHitsTop,trackHitsSide,aSrTr.GetChi2()*nass,aTr2,k);
...
void TADCactNtuTrack::sortDoubleHits(TADCntuRaw *hp, TADCntuTrack* pntr, TADCparGeo* ppg, TADCntuTrackTr &aSrTr, TADCntuTrackTr &RefTr, int add[36], int ndou)
{
...
void TADCactNtuTrack::refitImprovedTrackTop (TADCntuRaw *hp, TADCntuTrack* pntr, TADCparGeo* ppg, int mulT, int mulS, double trchi2, TADCntuTrackTr &aRTr, int j)
{
...
void TADCactNtuTrack::refitImprovedTrackSide(TADCntuRaw *hp, TADCntuTrack* pntr, TADCparGeo* ppg, int mulT, int mulS, double trchi2, TADCntuTrackTr &aRTr, int j)
{
